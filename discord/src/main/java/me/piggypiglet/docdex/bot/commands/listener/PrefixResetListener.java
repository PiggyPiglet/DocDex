...
public final class PrefixResetListener extends ListenerAdapter {
    ...
    @Inject
    public PrefixResetListener(@NotNull final Set<Server> servers, @NotNull @Named("default") final Server defaultServer,
                               @NotNull final DatabaseObjects databaseObjects) {
    ...
    public void onGuildMessageReceived(@NotNull final GuildMessageReceivedEvent event) {
        EXECUTOR.submit(() -> {
            ...
            final Server server = servers.stream()
                    .filter(element -> element.getId().equals(event.getGuild().getId()))
                    .findAny().orElse(null);
